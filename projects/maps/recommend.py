...
def group_by_centroid(restaurants, centroids):
    ...
    # BEGIN Question 4
    "*** REPLACE THIS LINE ***"
    location = [restaurant_location(i) for i in restaurants]
    # locations of restaurants
    close = [find_closest(i,centroids) for i in location]
    # closest centroids from each restaurants
    paired = zip(close, restaurants)
    # pair of closest centroids with restaurants
    return group_by_first(paired)
    # END Question 4
...
def find_centroid(cluster):
    ...
    xx , yy = [], []
    for i in cluster:
        xx.append(restaurant_location(i)[0])
        yy.append(restaurant_location(i)[1])
    return [mean(xx), mean(yy)]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    b, a, r_squared, s_xx, s_yy, s_xy = 0, 0, 0, 0, 0, 0  # REPLACE THIS LINE WITH YOUR SOLUTION
    xs_mean, ys_mean = mean(xs), mean(ys)
    for i in range(len(restaurants)):
        s_xx += (xs[i] - xs_mean)**2
        s_yy += (ys[i] - ys_mean)**2
        s_xy += (xs[i] - xs_mean) * (ys[i] - ys_mean)
    b = s_xy / s_xx
    a = ys_mean - b*xs_mean
