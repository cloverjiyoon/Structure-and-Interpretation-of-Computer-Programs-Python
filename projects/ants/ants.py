		...
		if insect.is_ant:
			...
			if self.ant is insect:
				if hasattr(self.ant, 'container') and self.ant.container:
					...
					if not insect.impostor and isinstance(insect, QueenAnt):
						return
					self.ant = None
			else:
				if  self.ant.container and hasattr(self.ant, 'container') and self.ant.ant is insect:
...
class Bee(Insect):
	...
	def blocked(self):
		...
		# BEGIN Problem 6A
		"*** REPLACE THIS LINE ***"
		return self.place.ant and self.place.ant.blocks
		# END Problem 6A
...
class Ant(Insect):
	...
	def can_contain(self, other):
		return not other.container and not self.ant and self.container
...
class ThrowerAnt(Ant):
	...
	def nearest_bee(self, hive):
		...
		while counter != self.minimum:
			counter += 1
			place = place.entrance

		while place.entrance is not hive and place.entrance and counter != self.maximum:
			if place.bees:
				break
			place = place.entrance
			counter += 1
...
class ScubaThrower(ThrowerAnt):
	"""ScubaThrower is ant that is watersafe"""
...
class HungryAnt(Ant):
	...
	While digest, the HungryAnt can't eat another Bee.
	"""
	name = 'Hungry'
	# BEGIN Problem 6B
	"*** REPLACE THIS LINE ***"
...
class QueenAnt(ScubaThrower):  # You should change this line
	...
	def __init__(self):
		...
		if QueenAnt.existence:
			self.impostor = True
		QueenAnt.existence = True
		# END Problem 9
	...
	def action(self, colony):
		...
		if self.impostor:
			...
			while place.exit:
				if place.ant:
					if place.ant not in self.times2:
						place.ant.damage *= 2
						self.times2.append(place.ant)

					if  isinstance(place.ant, TankAnt) or isinstance(place.ant, BodyguardAnt):
						if place.ant.ant and place.ant.ant not in self.times2:
							place.ant.ant.damage *= 2
							self.times2.append(place.ant.ant)
...
def apply_effect(effect, bee, gap):
	"""Apply a status effect to a BEE that lasts for gap turns."""
	# BEGIN Problem EC
	"*** REPLACE THIS LINE ***"
	bee.action = effect(bee.action, gap)
	# END Problem EC
