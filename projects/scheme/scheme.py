...
class Procedure:
	...
	def eval_call(self, operands, env, tail = False):
...
def do_define_form(expressions, env):
	...
	if scheme_symbolp(target):
		...
		if target.second == nil:
			val = LambdaProcedure(nil, expressions.second, env)
		else:
			val = LambdaProcedure(target.second, expressions.second, env)
...
def make_let_frame(bindings, env):
	...
	while bindings:
		prime = bindings.first
		check_form(prime, 2, 2)
		formals = Pair(prime.first, formals)
		values = Pair(scheme_eval(prime.second.first, env), values)
...
def do_mu_form(expressions, env):
	...
	# BEGIN PROBLEM 16
	"*** REPLACE THIS LINE ***"
	sec = expressions.second
	return MuProcedure(formals, sec)
	# END PROBLEM 16
...
def scheme_optimized_eval(expr, env, tail=False):
	...
	while isinstance(result, Thunk):
		...
		if (scheme_symbolp(first) and first in SPECIAL_FORMS):
			...
			eval_operands_func = lambda operands: scheme_eval(operands, env)
			operands = rest.map(eval_operands_func)
